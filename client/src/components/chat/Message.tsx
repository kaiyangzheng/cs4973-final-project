...
export default function Message({
  ...
  const isExtractionInProgress = message.metadata?.usingSelection && 
    (!message.metadata.selectionPreview || 
     message.metadata.selectionPreview === "Full document extraction in progress..." ||
    ...
    >
      {message.role === "user" ? (
        <div className="flex flex-col rounded-md bg-blue-500 p-3">
          ...
          {message.metadata?.usingSelection && (
            <div className={`text-xs ${isExtractionInProgress 
              ? 'bg-yellow-600' 
              : message.metadata?.isFullDocument 
                ? 'bg-green-600' 
                : 'bg-blue-600'} p-1 rounded mt-1 mb-2`}>
              {isExtractionInProgress ? (
                <span className="font-medium">Extracting document text...</span>
              ) : message.metadata?.isFullDocument ? (
                <span className="font-medium">Using entire document ({message.metadata.selectionLength} characters)</span>
              ) : (
                <>
        ...
        <div className="rounded-md bg-white p-3 text-black">
          {paperCategories.length > 0 && (
            <div className="mb-3">
              <div className="text-xs text-gray-500 mb-1">Paper Categories:</div>
